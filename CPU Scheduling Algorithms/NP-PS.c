...
int min_priority(int arr[], int n, int fin[]){
...
int main(){
    ...
    int pid[n], at[n], bt[n], rbt[n], st[n], ft[n], wt[n], rt[n], tat[n], fin[n], p[n];
    ...
    int min_ind = min_priority(p, n, fin);
    ...
    printf("Gantt chart\n");
    printf("PID\tAT\tBT\tP\tST\tFT\tWT\tTAT\n");
    for (int i = 0; i < n; i++){
        printf("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", pid[i], at[i], bt[i], p[i], st[i], ft[i], wt[i], tat[i]);
        avg_w += wt[i];
        avg_tat += tat[i];
        avg_rt += rt[i];
    }
